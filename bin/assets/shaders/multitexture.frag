...
void main()
{
	...
	//FragColor = mix(texture(textura, data_in.TexCoords), texture(textura2, data_in.TexCoords), 0.5);
	...
	if(gl_FragCoord.z < 0.8)
		FragColor = texture(textura, data_in.TexCoords);
	else
		FragColor = texture(textura2, data_in.TexCoords);
	*/
	...
	*/
	...
	FragColor = mix(texture(textura, data_in.TexCoords), texture(textura2, data_in.TexCoords), mix);
	
	// d) Mezcla usando un color específico de la segunda textura (negro en este caso) como transparencia
	/*
	vec4 color1 = texture(textura, data_in.TexCoords);
	vec4 color2 = texture(textura, data_in.TexCoords);
